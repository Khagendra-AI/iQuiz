...
const Quiz1 = ({navigation}: {navigation: any}) => {
  const opArray=['+','-','*','/']
  ...
  const {level, name} = useSelector((state: any) => {
      ...
      <View style={styles.header}>
        ...
        <View style={styles.logoutView}>
          ...
          <TouchableOpacity
            ...
            <Image source={Icon.logout} style={styles.logoutImage} />
          </TouchableOpacity>
        </View>
      </View>
      <View style={styles.body}>
        ...
        <View style={styles.optionView}>
          <TouchableOpacity
            ...
            <Text style={styles.optionText}>{result(`${x}${operator}${y}`)}</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={styles.optionTouchable}
            onPress={onOption2Press}>
            <Text style={styles.optionText}>{result(`${x}${operator}${y}-1`)}</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={styles.optionTouchable}
            onPress={onOption3Press}>
            <Text style={styles.optionText}>{result(`${x}${operator}${y}-2`)}</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={styles.optionTouchable}
            onPress={onOption4Press}>
            <Text style={styles.optionText}>{result(`${x}${operator}${y}+1`)}</Text>
          </TouchableOpacity>
        </View>
